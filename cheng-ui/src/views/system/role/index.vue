<template>
   <div class="app-container">
      <el-form :model="queryParams" ref="queryRef" v-show="showSearch" :inline="true" label-width="68px">
         ...
         <el-form-item label="權限字串" prop="roleKey">
            <el-input
      ...
      <!-- 表格資料 -->
      <el-table v-loading="loading" :data="roleList" @selection-change="handleSelectionChange" class="custom-table">
         ...
         <el-table-column label="權限字串" prop="roleKey" :show-overflow-tooltip="true" width="150" />
         ...
         <el-table-column label="操作" align="center" width="150" class-name="small-padding fixed-width" fixed="right">
            <template #default="scope">
              <el-tooltip content="修改" placement="top" v-if="scope.row.roleId !== 1">
                <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="[SYSTEM_ROLE_EDIT]"></el-button>
              </el-tooltip>
              <el-tooltip content="刪除" placement="top" v-if="scope.row.roleId !== 1">
                <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="[SYSTEM_ROLE_REMOVE]"></el-button>
              </el-tooltip>
              <el-tooltip content="資料權限" placement="top" v-if="scope.row.roleId !== 1">
                <el-button link type="primary" icon="CircleCheck" @click="handleDataScope(scope.row)" v-hasPermi="[SYSTEM_ROLE_EDIT]"></el-button>
              </el-tooltip>
              <el-tooltip content="分配使用者" placement="top" v-if="scope.row.roleId !== 1">
                <el-button link type="primary" icon="User" @click="handleAuthUser(scope.row)" v-hasPermi="[SYSTEM_ROLE_EDIT]"></el-button>
              </el-tooltip>
            </template>
         </el-table-column>
      </el-table>
      ...
      <el-dialog :title="title" v-model="open" width="500px" append-to-body>
         <el-form ref="roleRef" :model="form" :rules="rules" label-width="100px">
            ...
            <el-form-item prop="roleKey">
               <template #label>
                  <span>
                     <el-tooltip content="控制器中定義的權限字串，如：@PreAuthorize(`@ss.hasRole('admin')`)" placement="top">
                        <el-icon><question-filled /></el-icon>
                     </el-tooltip>
                     權限字串
                  </span>
               </template>
               <el-input v-model="form.roleKey" placeholder="請輸入權限字串" />
            </el-form-item>
      ...
      <el-dialog :title="title" v-model="openDataScope" width="500px" append-to-body>
         <el-form :model="form" label-width="80px">
            ...
            <el-form-item label="資料權限" v-show="form.dataScope == 2">
...
const dataScopeOptions = ref([
  { value: "1", label: "全部資料權限" },
  { value: "2", label: "自定義資料權限" },
  { value: "3", label: "本部門資料權限" },
  { value: "4", label: "本部門及以下資料權限" },
  { value: "5", label: "僅本人資料權限" }
])
...
const data = reactive({
  ...
  rules: {
    ...
    roleKey: [{ required: true, message: "權限字串不能為空", trigger: "blur" }],
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否確認刪除角色編號為"' + roleIds + '"的資料選項?').then(function () {
