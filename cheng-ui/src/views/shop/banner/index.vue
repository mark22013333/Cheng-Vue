<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="bannerList" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="imageUrl" label="輪播圖" width="200" align="center">
    ...
    <el-dialog :title="title" v-model="open" width="600px" append-to-body>
      <el-form ref="bannerFormRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="輪播圖片" prop="imageUrl">
          <image-upload v-model="form.imageUrl" :limit="1" />
          <div class="el-upload__tip">建議尺寸 750×300 像素</div>
        </el-form-item>
        <el-form-item label="連結類型" prop="linkType">
          <el-radio-group v-model="form.linkType" @change="handleLinkTypeChange">
            <el-radio label="NONE">無連結</el-radio>
            <el-radio label="PRODUCT">商品詳情</el-radio>
            <el-radio label="URL">外部連結</el-radio>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item v-if="form.linkType === 'URL'" label="連結網址" prop="linkValue">
          ...
          <div class="el-upload__tip" style="color: #909399;">網址需以 http:// 或 https:// 開頭</div>
        </el-form-item>
        <el-row :gutter="20">
          <el-col :span="12">
...
const rules = {
  ...
  imageUrl: [{ required: true, message: '請上傳輪播圖片', trigger: 'change' }]
}
...
function handleStatusChange(row) {
  ...
  proxy.$modal.confirm('確認要「' + text + '」輪播「' + row.title + '」嗎？').then(() => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('確認刪除選中的輪播嗎？').then(() => {
