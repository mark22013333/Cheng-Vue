  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="Plus" @click="handleAdd"
                   v-hasPermi="[TAG_GROUP_LINE_ADD]">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete"
                   v-hasPermi="[TAG_GROUP_LINE_REMOVE]">刪除</el-button>
      </el-col>
      ...
      <el-table-column label="操作" width="250" align="center" fixed="right">
        <template #default="{ row }">
          <el-button link type="primary" icon="View" @click="handlePreview(row)"
                     v-hasPermi="[TAG_GROUP_LINE_QUERY]">預覽</el-button>
          <el-button link type="success" icon="Refresh" @click="handleCalc(row)"
                     v-hasPermi="[TAG_GROUP_LINE_CALC]">運算</el-button>
          <el-button link type="primary" icon="Edit" @click="handleUpdate(row)"
                     v-hasPermi="[TAG_GROUP_LINE_EDIT]">修改</el-button>
    ...
    <el-dialog title="群組運算預覽" v-model="previewOpen" width="500px" append-to-body>
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="符合數量">{{ previewData.matchCount }} 位使用者</el-descriptions-item>
      </el-descriptions>
      <el-divider content-position="left">符合的使用者 ID（前 10 筆）</el-divider>
      <el-tag v-for="id in previewData.matchedIds" :key="id" class="preview-tag" type="info">{{ id }}</el-tag>
      <el-empty v-if="!previewData.matchedIds?.length" description="沒有符合條件的使用者" />
    </el-dialog>
    ...
    <el-dialog title="群組運算" v-model="calcOpen" width="400px" append-to-body :close-on-click-modal="false">
      ...
      <div v-else class="calc-result">
        <el-result :icon="calcResult.success ? 'success' : 'error'"
          ...
          <template #sub-title>
            <p v-if="calcResult.success">符合群組的使用者數量：<strong>{{ calcResult.matchCount }}</strong></p>
...
import {
  ...
  TAG_GROUP_LINE_CALC,
  TAG_GROUP_LINE_EDIT,
  TAG_GROUP_LINE_QUERY,
  TAG_GROUP_LINE_REMOVE
} from '@/constants/permissions'
defineOptions({ name: 'LineTagGroup' })
...
import { getLineTagOptions } from '@/api/tag/line'
...
function getList() {
  loading.value = true
  listLineTagGroups(queryParams.value).then(response => {
...
function handleUpdate(row) {
  ...
  getLineTagGroup(groupId).then(response => {
...
function submitForm() {
  proxy.$refs['formRef'].validate(valid => {
    ...
    const action = form.value.groupId ? updateLineTagGroup : addLineTagGroup
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('確定要刪除選取的群組嗎？').then(() => {
    return delLineTagGroup(groupIds.join(','))
  }).then(() => {
    getList()
...
function handlePreview(row) {
  ...
  previewLineTagGroup(row.groupId, 10).then(response => {
    previewData.value = {
...
function handleCalc(row) {
  ...
  calcLineTagGroup(row.groupId).then(response => {
