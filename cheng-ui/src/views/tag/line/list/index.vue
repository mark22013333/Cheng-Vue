<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="tagList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="使用者數" align="center" prop="userCount" width="100" />
...
function getList() {
  loading.value = true
  listLineTags(queryParams.value).then(response => {
  ...
  formRef.value.validate(valid => {
    if (valid) {
      if (form.value.tagId != null) {
        updateLineTag(form.value).then(response => {
          proxy.$modal.msgSuccess('修改成功')
          open.value = false
          getList()
        })
      } else {
        addLineTag(form.value).then(response => {
