<template>
  <div class="container">
    <div class="left-board">
      ...
      <el-scrollbar class="left-scrollbar">
        <div class="components-list">
          ...
          <div class="components-title">
            <svg-icon icon-class="component" /> 版面型元件
          </div>
    ...
    <code-type-dialog v-model="dialogVisible" title="選擇產生類型" :showFileName="showFileName" @confirm="generate" />
...
function empty() {
  proxy.$modal.confirm('確定要清空所有元件嗎？', '提示', { type: 'warning' }).then(() => {
      idGlobal.value = 100
      drawingList.value = []
    }
  )
...
onMounted(() => {
  const clipboard = new ClipboardJS('#copyNode', {
    text: trigger => {
      const codeStr = generateCode()
      ElNotification({ title: '成功', message: '程式碼已複製到剪切板，可複製貼上。', type: 'success' })
      return codeStr
    }
  })
  clipboard.on('error', e => {
    proxy.$modal.msgError('程式碼複製失败')
  })
})
</script>
